...
{
    ofstream guardarArchi(nombre_archivo.c_str());
    guardarArchi<<pais<<endl;
    guardarArchi.close();
...
{
    ifstream abrirArchi(nombre_archivo.c_str());

    string valor;
    vector<string> temp;

    while(abrirArchi>>valor){
        temp.push_back(valor);
    }
    abrirArchi.close();
    //Pais *arr = new string [temp.size()];
